...
vi G[200020];
ll n;
bool mainstream[200020];
int starti = 0, endi = 0, maxDist = 0;

ll ret[200020];
bool stepping[200020];
...
ll dfs(ll cur, ll par, ll su) {
    ...
    rep(i, G[cur].size()) {
        ...
        if (mainstream[G[cur][i]]) {
            temp = G[cur][i];
            continue;
        }
        su++;
        su = dfs(G[cur][i], cur, su);
    }
    if (temp >= 0) {
        su++;
        dfs(temp, cur, su);
...
signed main() {
    ...
    treeDFS(-1, starti, 0, maxDist, endi);
    starti = endi, endi = 0, maxDist = 0;
    treeDFS(-1, starti, 0, maxDist, endi);
    ...
    // printf("start: %lld, end: %lld, diameter: %lld\n", start, end, maxDist);
