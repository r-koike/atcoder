...
// x^n (mod M)を返す
// O(log n)
ll modpow(ll x, ll nt) {
    if (nt == 0)
        return 1;
    if (nt % 2 == 0)
        return modpow(x * x % M, nt / 2);
    else
        return x * modpow(x, nt - 1) % M;
}
...
const ll MAX_KAIJO_N = 1000;    // n%mの最大値?
...
signed main() {
    ...
    rep1(i, 52) {
        ...
        ll x1 = kaijo[i];
        rep(j, i) {
            ll rett = modcomb(i, j, M) * dp[j];
            // if (i == 3) {
            //     disp(modcomb(i, j, M), dp[j]);
            // }
            x1 -= rett;
            x1 = ((x1 % M) + M) % M;
        }
