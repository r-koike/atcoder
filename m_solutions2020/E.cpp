...
signed main() {
    ...
    rep(i, n) {
        ...
        omindist[i] = min(abs(x[i]), abs(y[i]));
    }

    rep(k, n + 1) {
        rep(i, n) {
            mindist[i] = omindist[i];
        }
        ...
        rep(michi, k) {
            ...
            bool xWoReduce = true;
            rep(i, n) {
                // xtに道を建設したときに減らすことのできる量を計算する
                ll xt = x[i];
                ll sum = 0;
                rep(j, n) {
                    ll nkyori = abs(xt - x[j]);
                    if (nkyori < mindist[j]) {
                        sum += (mindist[j] - nkyori) * p[j];
                    }
                }
                if (sum > maxReduce) {
                    xWoReduce = true;
            ...
            rep(i, n) {
                // ytに道を建設したときに減らすことのできる量を計算する
                ll yt = y[i];
                ll sum = 0;
                rep(j, n) {
                    ll nkyori = abs(yt - y[j]);
                    if (nkyori < mindist[j]) {
                        sum += (mindist[j] - nkyori) * p[j];
                    }
                }
            ...
            if (xWoReduce) {
                ...
                rep(j, n) {
                    ll nkyori = abs(xt - x[j]);
                    mindist[j] = min(nkyori, mindist[j]);
                }
            } else {
                ll yt = y[maxReduceId];
                rep(j, n) {
