...
signed main() {
    ...
    rep(i, n) {
        ...
        vec.pb(mp(i, mp(a, mp(r - 1, c - 1))));
    }

    // ===================### ソート関係 ###=================== //
    // 第二要素が大きい順
    sort(vec.begin(), vec.end(),
         [](const pair<int, pair<int, pair<int, int>>> a,
    ...
    int veci = 0;
    while (1) {
        int cura = vec[veci].sc.fr;
        vector<pair<int, pair<int, int>>> curp;
        while (cura == vec[veci].sc.fr) {
            curp.pb(mp(vec[veci].fr, vec[veci].sc.sc));
            veci++;
            if (veci >= vec.size()) {
                break;
            }
        ...
        vector<pair<int, pair<int, int>>> quevec;
        rep(i, curp.size()) {
            int j = curp[i].fr;
            int r = curp[i].sc.fr;
            int c = curp[i].sc.sc;
            // 最大値のsetたちを更新する
            int t = max(maxr[r], maxc[c]) + 1;
            ret[j] = t;
            quevec.pb(mp(t, mp(r, c)));
        }

        rep(i, quevec.size()) {
            int t = quevec[i].fr;
            int r = quevec[i].sc.fr;
            int c = quevec[i].sc.sc;
            maxr[r] = max(maxr[r], t);
            maxc[c] = max(maxc[c], t);
        }
