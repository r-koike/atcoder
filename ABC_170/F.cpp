...
vector<vector<pair<int, vector<P>>>> dp;

signed main() {
    scanf("%d%d%d", &h, &w, &k);
    scanf("%d%d%d%d", &x1t, &y1t, &x2t, &y2t);
    ...
    rep(i, h) {
        string s = "";
        vector<pair<int, vector<P>>> dpt;
        rep(j, w) {
            char c;
            scanf(" %c", &c);
            s.pb(c);

            dpt.pb(mp(INF, vp{mp(0, 0)}));
        }
        field.pb(s);
        dp.pb(dpt);
    }
    ...
    while (!que.empty()) {
        ...
        int y = p.sc;
        int cost = dp[x][y].fr;
        int dirPow[4] = {0, 0, 0, 0};
        rep(i, dp[x][y].sc.size()) {
            dirPow[dp[x][y].sc[i].fr] = dp[x][y].sc[i].sc;
        }
        ...
        rep(i, 4) {
            ...
            if (nx < 0 || h - 1 < nx || ny < 0 || w - 1 < y || field[nx][ny] == '@') {
                continue;
            }
            if (dp[nx][ny].fr < cost) {
                continue;
            ...
            bool dirnew = true;
            rep(j, dp[nx][ny].sc.size()) {
                if (dp[nx][ny].sc[j].fr == i && dp[nx][ny].sc[j].sc <= npo) {
                    dirnew = false;
                }
            }
            if (dp[nx][ny].fr > ncost || dirnew) {
                que.push(mp(nx, ny));
                dp[nx][ny].fr = ncost;
                dp[nx][ny].sc.pb(mp(i, npo));
            }
        }
    }

    int ret = dp[x2t][y2t].fr;
    // rep(i, h) {
    //     rep(j, w) {
    //         int a = dp[i][j].fr;
    //         if (a == INF) {
