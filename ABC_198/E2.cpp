...
signed main() {
    ...
    do {
        ...
        int t1 = 1;
        rrep(i, s1.size()) {
            x1 += order[dp[s1[i] - 'a']] * t1;
            t1 *= 10;
        }
        int x2 = 0;
        int t2 = 1;
        rrep(i, s2.size()) {
            x2 += order[dp[s2[i] - 'a']] * t2;
            t2 *= 10;
        }
        int x3 = 0;
        int t3 = 1;
        rrep(i, s3.size()) {
            x3 += order[dp[s3[i] - 'a']] * t3;
            t3 *= 10;
        }

        bool esc = false;
        if (s1.size() > 1) {
            int temp = order[dp[s1[0] - 'a']];
            if (temp == 0) {
                continue;
            }
        }
        if (s2.size() > 1) {
            int temp = order[dp[s2[0] - 'a']];
            if (temp == 0) {
                continue;
            }
        }
        if (s3.size() > 1) {
            int temp = order[dp[s3[0] - 'a']];
            if (temp == 0) {
                continue;
            }
        }
        if (x3 == x1 + x2 && x1 > 0 && x2 > 0 && x3 > 0) {
            rep(i, s1.size()) {
                printf("%lld", order[dp[s1[i] - 'a']]);
            }
            printf("\n");
            rep(i, s2.size()) {
                printf("%lld", order[dp[s2[i] - 'a']]);
            }
            printf("\n");
            rep(i, s3.size()) {
                printf("%lld", order[dp[s3[i] - 'a']]);
            }
            printf("\n");
            return 0;
        }
