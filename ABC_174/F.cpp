...
signed main() {
    ...
    // ===================### ソート関係 ###=================== //
    // lが小さい順
    sort(query.begin(), query.end(), [](const P1 a, const P1 b) {
        if (a.sc.fr != b.sc.fr)
    ...
    int qi = 0;
    rep1(lnow, n) {
        int l = query[qi].sc.fr;
        int r = query[qi].sc.sc;
        while (l == lnow) {
            // set内でrより大きいのの個数をtempに
            std::multiset<int>::iterator iter1 = ub(set, r);
            std::multiset<int>::iterator iter2 = set.end();
            int temp = iter2 - iter1;
            ret[query[qi].fr] = sum[r];
        ...
        // ここにlnowを進めてsetに挿入の処理
        set.insert(tugi[lnow]);
    }
