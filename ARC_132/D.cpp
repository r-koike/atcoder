...
/* ------------------------------------- */
char str_temp0[300030];
char str_temp1[300030];
int dp[300030][2];
    ...
    rep(i, sl) {
        ...
        if (cnt0 == cnt1) {
            ...
            if (s[i] == '1' && t[i] == '1') {
                // lenは1になっているはず
                dp[num][0] = -INF;
                dp[num][1] = max(dp[num - 1][0], dp[num - 1][1] + 1);
            } else if (s[i] == '1' && t[i] == '1') {
                // lenは1になっているはず
                dp[num][0] = max(dp[num - 1][0] + 1, dp[num - 1][1]);
                dp[num][1] = -INF;
            } else {
                if ((ren0 == len && s[prei] == '1') || (ren1 == len && t[prei] == '1')) {
                    // 1111100000が使える
                    dp[num][0] = max(dp[num - 1][1] + (i - prei - 2) + 1,
                                     dp[num - 1][0] + (i - prei - 2));
                }
                if ((ren0 == len && s[i] == '1') || (ren1 == len && t[i] == '1')) {
                    // 0000011111が使える
                    dp[num][1] = max(dp[num - 1][0] + (i - prei - 2) + 1,
                                     dp[num - 1][1] + (i - prei - 2));
                }
                dp[num][0] = max(dp[num][0], max(dp[num - 1][0]));
