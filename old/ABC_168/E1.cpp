...
signed main() {
    ...
    rep(i, n) {
        ...
        auto iter0 = lower_bound(vec.begin(), vec.end(), p1, cmp);
        auto iter1 = upper_bound(vec.begin(), vec.end(), p1, cmp);

        ll num = iter1 - iter0;
        if (p0.fr * p0.fr + p0.sc * p0.sc == 0) {
            num--;
        }
        d[num]++;
        if (num > 1) {
    ...
    rep1(i, n) {
        ll a = ni_ruijo[i + 1] - (ni_ruijo[i] - 1);
        rep(j, d[i]) {
            ret *= a;
            ret %= M;
        }
