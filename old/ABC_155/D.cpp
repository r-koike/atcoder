...
signed main() {
    ...
    ll res_neg = a_neg * a_pos;
    ll res_pos = a_neg * (a_neg - 1) / 2 + a_pos * (a_pos - 1) / 2;
    ll res_zero = a_zero * (a_neg + a_pos) + a_zero * (a_zero - 1) / 2;
    // disp(res_neg + res_pos + res_zero);
    // disp(n * (n - 1) / 2);

    if (k <= res_neg) {
        // k番目に小さい負の数を出力する
        ll ret = neg[neg.size() - 1] * pos[pos.size() - 1] / 2;
        while (1) {
            ll kt = 0;
            rep(i, neg.size()) {
                auto iter = lb(pos, ret / neg[i]);
                kt += iter - pos.begin();
            }
            if (kt == k) {
                break;
    ...
    } else if (k <= res_neg + res_zero) {
        printf("%d\n", 0);
    } else {
        // k-(res_neg + res_zero)番目の正の数を出力する
    }
