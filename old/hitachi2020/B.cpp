...
signed main() {
    int a[100010], b[100010], M, A, B, x[100010], y[100010], c[100010];
    scanf("%d%d%d", &A, &B, &M);
    ...
    rep(i, M) {
        ...
        x[i]--;
        y[i]--;
        int kane = max(0, a[x[i]] + b[y[i]] - c[i]);
        warimin = min(kane, warimin);
    }

    printf("%d\n", min(warimin, amin + bmin));
