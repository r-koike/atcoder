...
// j以降を全てflip
vp flip(vp vec, int j) {
    vp vec1 = vec;
    for (int i = j + 1; i < vec.size(); i += 2) {
        vec1[i] = mp(vec[i].sc, vec[i].fr);
        vec1[i + 1] = mp(vec[i + 1].sc, vec[i + 1].fr);
    }
    return vec1;
}

// jにpを挿入する
vp iflip(vp vec, int j, P p) {
    vp vec1 = vec;
    vec1.pb(p);
    vec1[j] = p;
    for (int i = j + 1; i < vec.size(); i += 2) {
...
void dfs(int i, int sum, vector<P> vec) {
    if (i == 0) {
        vp vec1 = vec;
        vec1.pb(mp(a[i], b[i]));
        dfs(1, 0, vec1);
    ...
    for (int j = 0; j < i / 2; j++) {
        vp vec1 = vec;
        P p = mp(a[i], b[i]);
        dfs(i + 1, sum + 2 * j, iflip(vec1, i - j, p));
    }
    return;
}
