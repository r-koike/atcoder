...
signed main() {
    ...
    for (ll i = 2; i < n; i++) {
        ...
        rep(j, i) {
            ll dist = 0;
            dist += abs(x[vec[j]] - x[i]) + abs(y[vec[j]] - y[i]);
            dist += abs(x[vec[j + 1]] - x[i]) + abs(y[vec[j + 1]] - y[i]);
            dist += max(0ll, z[i] - z[vec[j]]);
            dist += max(0ll, z[vec[j + 1]] - z[i]);
            if (mind > dist) {
                mind = dist;
                minj = j;
            }
    ...
    rep(i, vec.size() - 1) {
        ...
        ll q = y[vec[i + 1]];
        ll r = z[vec[i + 1]];
        ret += abs(p - a) + abs(q - b) + max(0ll, r - c);
    }
