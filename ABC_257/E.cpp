...
P func(int &nokori) {
    ...
    rep1(i, 9) {
        int keta = nokori / c[i];
        int nok = nokori - c[i] * keta;
        if (keta > maxKeta || (keta == maxKeta && nok > maxNokori)) {
            if (nokori >= 0) {
                maxKeta = keta;
                maxNokori = nok;
                bestId = i;
...
signed main() {
    ...
    ll finalRet = ret[ret.size() - 1];
    for (ll i = 9; i > finalRet; i--) {
        // if( < c[finalRet] + nokori)
    }
