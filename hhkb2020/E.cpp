...
signed main() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++)
            if (mapp[i][j] == '.')
                f1[i][j] = f1[i - 1][j] + 1, f2[i][j] = f2[i][j - 1] + 1;
    for (int i = n; i; i--)
        for (int j = m; j; j--)
            if (mapp[i][j] == '.')
                f3[i][j] = f3[i + 1][j] + 1, f4[i][j] = f4[i][j + 1] + 1;

    rep1(i, n) {
        rep1(j, m) {
            if (mapp[i][j] == '.')
                dp[i - 1][j - 1] = f1[i][j] + f2[i][j] + f3[i][j] + f4[i][j] - 3;
            else
                dp[i - 1][j - 1] = INF;
        }
    }
