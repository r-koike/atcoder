    ...
    rep(i, r) {
        rep(j, c) {
            if (i < r - 1) {
                ...
                dp[i + 1][j][0] = max(dp[i + 1][j][0], ma);
                if (item[i + 1][j] > 0) {
                    dp[i + 1][j][1] = max(dp[i + 1][j][1], ma + item[i + 1][j]);
                }
            }
            if (j < c - 1) {
                ...
                if (item[i][j + 1] > 0) {
                    dp[i][j + 1][1] =
                        max(dp[i][j + 1][1], max(dp[i][j][1], dp[i][j][0] + item[i][j + 1]));
                    dp[i][j + 1][2] =
                        max(dp[i][j + 1][2], max(dp[i][j][2], dp[i][j][1] + item[i][j + 1]));
                    dp[i][j + 1][3] =
                        max(dp[i][j + 1][3], max(dp[i][j][3], dp[i][j][2] + item[i][j + 1]));
                } else {
                    dp[i][j + 1][1] = max(dp[i][j + 1][1], max(dp[i][j][1], -INF));
                    dp[i][j + 1][2] = max(dp[i][j + 1][2], max(dp[i][j][2], -INF));
                    dp[i][j + 1][3] = max(dp[i][j + 1][3], max(dp[i][j][3], -INF));
                }
            }
        }
    }
