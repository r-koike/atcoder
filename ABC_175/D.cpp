...
signed main() {
    ...
    rep(i, n) {
        ...
        if (k > nt) {
            if (nt != n) {
                su1 = max(masu + su * (((k - 1) / nt) - 1), masu) - c[i];
            } else {
                su1 = max(masu + su * (((k - 1) / nt) - 0), masu) - c[i];
            }
        }
        su2 = max(mask + su * ((k - 1) / nt), mask) - c[i];
        ll rett = max(su1, su2);
        disp(rett, su1, su2, masu, mask, su);
        // disp(rett);
        ma = max(rett, ma);
    }
    printf("%lld\n", ma);
